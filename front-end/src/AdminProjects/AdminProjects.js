...
function AdminProjects()
{
  ...
  useEffect(()=> 
  {
    const fetchProjects=async()=> 
    {
      try 
      {
        const response=await axios.post('http://localhost:8081/home/get_project');
  ...
  const handleSubmit = async (e) =>
  {
    ...
    try 
    {
        axios.post('http://localhost:8081/home/project/add_new_project',values).then(res => 
        {
  ...
  return (
    <div>
        ...
        <Row className='d-flex justify-content-center ml-auto mr-auto mb-2'>
            <h2 style={{ textAlign: 'center' }}>List of Projects</h2>
        </Row>
        ...
        <Modal show={showOpen} onHide={handleCreateNewCustomersShowClose} className="d-flex justify-content-center align-items-center vh-100">
            <form onSubmit={handleSubmit}> 
              <Modal.Header closeButton>
                <Modal.Title><center>Create New Project</center></Modal.Title>
              </Modal.Header>
              <Modal.Body>

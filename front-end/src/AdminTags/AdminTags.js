...
function AdminTags()
{
  ...
  useEffect(()=> 
  {
    const fetchProjects=async()=> 
    {
      try 
      {
        const response=await axios.post('http://localhost:8081/home/get_tag');
  ...
  const handleSubmit = async (e) =>
  {
    ...
    try 
    {
        axios.post('http://localhost:8081/home/tag/add_new_tag',values).then(res => 
        {
            if(res.data.message === "Tag created successfully.") 
            {
              alert("Your Tag has been created successfully.");
    ...
    <div>
        <Row className='d-flex justify-content-center ml-auto mr-auto mt-5 mb-2'>
            <button className="btn btn-primary w-20 fw-bold text-center align-items-center" title='Click Here to Create New Customer' onClick={handleCreateNewCustomersShow}>
              <FontAwesomeIcon icon={faPlus} /> Create New Tag
            </button>
        </Row>
        <Row className='d-flex justify-content-center ml-auto mr-auto mb-2'>
            <h2 style={{ textAlign: 'center' }}>List of Tags</h2>
        </Row>
        ...
        <Modal show={showOpen} onHide={handleCreateNewCustomersShowClose} className="d-flex justify-content-center align-items-center vh-100">
            <form onSubmit={handleSubmit}> 
              <Modal.Header closeButton>
                <Modal.Title><center>Create New Tag</center></Modal.Title>
              </Modal.Header>
              <Modal.Body>
